...
PS.init = function( system, options ) {
	...
	gameTimer = PS.timerStart(14, function(){
		seed = Number(PS.elapsed())
		PS.seed(seed);
		respawn = PS.random(18)
		respawn2 = PS.random(18)
		...
		if(PS.spriteMove(carbattery_red).y < 18) {
			...
			setTimeout(function(){
				PS.spriteMove(carbattery_red, respawn2,1)
				charge--
				//PS.recolor()
			},15)
...
PS.statusText("FULLY CHARGED! YOU WIN")
...
if (charge <= 0){
	PS.timerStop(gameTimer)
	PS.statusText("OUT OF CHARGE! GAME OVER!")
...
PS.keyDown = function( key, shift, ctrl, options ) {
	...
	if(key === 113) {
		...
		PS.spriteSolidColor(eel,PS.COLOR_YELLOW)
	...
	if(key === 101) {
		...
		PS.spriteSolidColor(eel,PS.COLOR_RED)
...
PS.recolor = function(){
	if (scorefilter===1) {
		...
		PS.statusText("BATTERIES COLLECTED: " + scoretext + " CHARGE LEFT: " + charge)
	}
	if (scorefilter===2) {
		PS.color(PS.ALL, PS.ALL, 0x5A5D4B)
		PS.alpha(PS.ALL, PS.ALL, 255)
	...
	if (scorefilter===3) {
		PS.color(PS.ALL, PS.ALL, 0xFFD6D1)
		PS.alpha(PS.ALL, PS.ALL, 255)
